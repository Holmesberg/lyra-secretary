"use client";

import Image from "next/image";
import { signIn } from "next-auth/react";

export function DeployCta() {
  return (
    <section
      id="deploy"
      className="relative overflow-hidden border-t border-hairline py-28 md:py-36"
    >
      <div
        aria-hidden
        className="pointer-events-none absolute left-1/2 top-[-10%] -z-10 h-96 w-[80%] -translate-x-1/2 rounded-full bg-signal/10 blur-3xl"
      />
      <div className="mx-auto max-w-2xl px-6 text-center md:px-10">
        <div className="mb-8 flex justify-center">
          <Image
            src="/lyraos-logo.png"
            alt="LyraOS"
            width={96}
            height={96}
            className="h-20 w-20 md:h-24 md:w-24"
          />
        </div>

        <h2 className="text-3xl font-medium leading-[1.1] tracking-tight text-parchment md:text-5xl">
          The instrument is live.
        </h2>

        <p className="mx-auto mt-5 max-w-md text-base leading-relaxed text-dust md:text-[17px]">
          Pre-alpha. Free. Your data stays in Supabase (eu-west-1). Export or
          delete anytime.
        </p>

        <button
          onClick={() => signIn("google", { callbackUrl: "/today" })}
          className="pill pill-primary mt-10 px-7 py-3.5 text-base focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal/70"
        >
          <GoogleGlyph className="h-4 w-4" />
          Sign in with Google
          <span aria-hidden>→</span>
        </button>

        <p className="mt-6 text-xs text-dust-deep">
          No credit card · No waitlist · Signup = first session
        </p>
      </div>
    </section>
  );
}

function GoogleGlyph({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" className={className} aria-hidden>
      <path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
        fill="#4285F4"
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill="#34A853"
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
        fill="#FBBC05"
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        fill="#EA4335"
      />
    </svg>
  );
}
