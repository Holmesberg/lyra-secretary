...
| **2** | JARVIS discovery soak | Hypothesis log shows **≥3 PROMOTED** and **≥2 REJECTED**, rejections show **discrimination** (not silent agrees) | `docs/jarvis_hypothesis_log.md` — count **Promoted** vs **Rejected** sections; until counts met, gate **blocks** Phase 3 |
| **3** | Inference engine (expand) | **`docs/inference_engine_architecture.md`** stays in sync with code **when** adding new user-facing writers or new aggregate fields | **DRAFT** on `main` — update same PR whenever inference outputs or signature keys change |
