...
- **LYR-111: Timer banner snaps to last whole minute after multi-task swap (visual only).** Operator-observed 2026-04-26 during swap dogfood: when resuming task A after a swap (A → B → swap back to A), the active-timer banner's running clock reboots from the integer-minute count and ticks forward — losing the 0–59 seconds of sub-minute remainder that A had at pause time. So a task paused at 5:32 elapsed resumes showing 5:00, then ticks to 6:00 a few seconds later, etc. Underlying `total_paused_minutes` math is float-precise (LYR-094) so research data unaffected — this is purely a banner-display artifact. Root cause: `StopwatchManager._active_elapsed()` at `backend/app/services/stopwatch_manager.py:268` returns `int(active_seconds // 60)` and the status payload's `elapsed_minutes` follows. Frontend banner uses that integer as its tick basis. Fix options: (a) add `elapsed_seconds: int` to the status response alongside `elapsed_minutes`; banner uses seconds for basis. (b) return `elapsed_minutes` as float (more invasive — touches every consumer of the status endpoint, including SKILL.md-driven OpenClaw tools that assume integer). Option (a) is the cheaper path. Defer behind higher-priority work. *Found Apr 26.*
