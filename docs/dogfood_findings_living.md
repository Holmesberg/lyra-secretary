...
### OPEN — Bug class: ghost-paused tasks + multi-tasking UX gap (Apr 25)

- **LYR-NNN ghost-paused tasks (PAUSED state, no open StopwatchSession).** Reproduced Apr 25 19:00. **Root cause:** `stale_session_recovery` (12h threshold, `backend/app/workers/jobs/stale_session_recovery.py`) closes the StopwatchSession but never transitions `Task.state` away from PAUSED. `orphan_task_recovery` (`backend/app/workers/jobs/orphan_task_recovery.py`) only catches `state==EXECUTING` tasks, not PAUSED. Result: a paused task held >12h becomes a permanent ghost — Stop button shows in UI (state-only logic) but `/v1/stopwatch/stop` returns "no active stopwatch" because Redis was cleared by stale_session_recovery and the session is auto_closed. **Confirmed cohort impact (Apr 25 audit):** u5 Altium (now operator-voided post-discovery), u6 "Compilers Lecs" still ghost-paused since Apr 21 03:23. **Fix scope:** extend `orphan_task_recovery._run_for_one_user` to also catch `state==PAUSED` with no open session → transition to SKIPPED with `initiation_status='orphaned_recovery'`. One-line filter change. Test: add `test_paused_task_no_open_session_recovers` mirroring the EXECUTING case. Defense-in-depth follow-up: `stale_session_recovery` itself should set `task.state = SKIPPED + initiation_status='orphaned_recovery'` when closing the orphan session, so the recovery happens in a single pass instead of two-job-tick latency. *Apr 25 audit.*

- **LYR-NNN multi-tasking parent-task swap not exposed.** When operator runs an interruption flow (`stopwatch_manager.start_as_interruption()` at line 348) — parent task PAUSED with open session, child task EXECUTING with `parent_task_id` set — there is currently no UX affordance to switch back to the parent without stopping the child. Active timer banner shows only the child; parent paused-with-open-session task in `/today` list shows no "Resume" button (the existing PAUSED state is visually identical to a planned-but-not-started task). **Confirmed live:** operator currently has "CO vid 2" PAUSED with open session + parent_task_id null, "feedback calibration" EXECUTING with parent_task_id pointing to CO vid 2 — exactly the multi-tasking pattern. **Fix scope:** new endpoint `POST /v1/stopwatch/switch/{task_id}` (or extend `/v1/stopwatch/resume` with optional target_task_id). Service method `switch_to_paused_task(target_task_id)`: pause currently-EXECUTING task in same transaction (close current pause_event, mark state PAUSED), find target's open session (must exist + same user), resume target (set Redis active, close target's pause_event, state EXECUTING). Frontend: in `/today` list, paused-with-open-session tasks render a distinct "Resume" button vs no-session paused tasks (which won't exist after LYR-NNN above lands). Optional banner-level: "Other in-progress: [parent-title]" chip below active timer with one-tap switch. **Operator's two proposed options:** (1) show two timers simultaneously — rejected, conceptually false (one CPU per human), (2) creative — recommendation is the swap affordance above. *Apr 25 audit.*
...
- **LYR-107: Rule 13 operational-filter drift — endpoint uses `> 0` instead of pre-registered `>= 5`.** Day-18 sweep: `analytics.py` `/bias_factor/lookup` handler filters tasks with `planned_duration_minutes > 0`, but MANIFESTO v1.10 Rule 13 operational definition of `n_sessions_in_cell` specifies `planned_duration_minutes >= 5`. Currently zero tasks leak through (no 1–4-min planned tasks exist), so this is a theoretical-only pre-registration divergence today. The moment any user plans a sub-5-min task it becomes a real Rule 13 violation affecting the canonical bias_factor computation. Fix: change the endpoint predicate to `>= 5`. Code-first fix (no amendment needed — code should match the pre-registered text, not the other way around). *Found Apr 23.*
...
- **LYR-105: Stuck-open pause_event when session closes while paused.** Day-18 sweep found 5 pause_event rows (u=1) with `resumed_at_utc IS NULL` while the parent `stopwatch_session.end_time_utc` is stamped — 4 from 2026-04-15 evening (all `external_interruption`, gaps 0–2min), 1 from 2026-04-22 21:15 (`distraction`, 121-min gap until session close). Root cause hypothesis: `stopwatch_manager.stop()` closes the session but doesn't close any dangling open pause_event. Effect: `duration_minutes` on the pause_event stays NULL, VT-17 pause-duration aggregation silently misses these rows. Fix: on session-close path, detect and close any open pause_event with `resumed_at_utc = session.end_time_utc`, `duration_minutes = (end − paused).seconds / 60`. One-liner in `stopwatch_manager.py` + test covering "stop while paused." *Found Apr 23.*

- **LYR-106: Negative-duration pause event — timestamp integrity guard missing.** Day-18 sweep found 1 pause_event (u=5, pe=a3c8629f, 2026-04-18) with `duration_minutes = -12.02` — the `resumed_at_utc` (23:10) is stamped BEFORE `paused_at_utc` (23:22). Single row, but the write path has no invariant check. Root cause unknown (clock skew / mid-session timezone change / race in resume handler). Fix: at write-time in `stopwatch_manager.py` resume path, reject or clamp when `resumed_at_utc < paused_at_utc`; log with surrounding session state so future instances are diagnosable. Backfill: void or zero the negative row. *Found Apr 23.*
...
- **LYR-110: `micro_mirror` is being dismissed 95% of the time.** Day-18 sweep across u=1/u=4/u=5/u=6: 20 of 21 viewed micro_mirror impressions ended in dismissal, avg_dwell 6.6s (glance + close). The surface is not delivering engagement. Options: (i) kill the surface and route the content to `/insights` instead; (ii) redesign the micro_mirror content format (current cards may be too abstract/insight-dense for a 6-second scan); (iii) A/B-test content shapes behind a flag. Pre-decision: do nothing for 48h, re-measure on 2026-04-25 — if still ≥90% dismissal at higher n, ship option (i). Post-mortem required regardless (LYR-098 shipped this surface with high expectations; we need to understand the miss). *Found Apr 23.*
