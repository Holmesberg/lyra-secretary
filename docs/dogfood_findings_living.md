...
*Ordering follows `docs/archive/legacy/planning/building_phases.md` §Phase 4.5 tier structure: Tier 1 retention architecture first (shipping gate), Tier 2 correctness fixes second. The order below reflects that prioritization.*
...
- **calibration_nudge at task creation (D3 seed).** When user sets `planned_duration_minutes` that predicts ≥25% overrun, surface pre-commit nudge with three affordances (keep / adjust to prediction / dismiss). Gated: (user × category) session count ≥ 10 before firing; "Insights unlock in N sessions" progress framing shown otherwise. Design spec: `docs/archive/legacy/planning/phase_6_architecture_backlog.md` §"Calibration Nudge at Task Creation (D3)". *Locked Apr 14.*
...
- **Canonical phase documentation created (Apr 14).** `docs/archive/legacy/history/project_history.md` (retrospective: what happened) and `docs/archive/legacy/planning/building_phases.md` (forward-looking: what remains). Updated at phase boundaries. Items from this dogfood doc graduate to building_phases when they solidify into phase-level commitments. Neither document duplicates dogfood — they reference it.

- **Phase 6 architecture backlog document recommended.** Create `docs/archive/legacy/planning/phase_6_architecture_backlog.md` with schemas and acceptance criteria for all Phase 6 P2 items (prediction-first logging, falsification engine, layered adaptation, intervention tracking, cascade model, metacognitive reliability, archetype re-fit, trigger field). 20 minutes one-time work, prevents rediscovering design decisions next month. *Apr 12.*
