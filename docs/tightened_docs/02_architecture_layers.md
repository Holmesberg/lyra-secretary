...
| operator tooling | parked `jarvis_tools.py`, `behavioral_signature_service.py`, `analytics/behavioral_signature`, notebooks | powerful/prototype | High risk of AI-generated semantic sediment; behavioral signature call sites now route through the service boundary |
...
- `behavioral_signature_for_operator()` previously delegated back into JARVIS
  ...
  aggregate. The 2026-07-10 R4 boundary seam moved that call site behind
  `behavioral_signature_service.py`; the large historical implementation body
  still remains parked in `jarvis_tools.py` until a later extraction.
