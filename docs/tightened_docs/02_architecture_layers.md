...
| operator tooling | parked `jarvis_tools.py`, `behavioral_signature_service.py`, `behavioral_signature_aggregate.py`, `analytics/behavioral_signature`, notebooks | powerful/prototype | High risk of AI-generated semantic sediment; behavioral signature call sites now route through the service boundary |
...
- `behavioral_signature_for_operator()` previously delegated back into JARVIS
  tools, making `inference_engine.py` depend on operator tooling for an
  aggregate. The 2026-07-10 R4 boundary seams moved that call site behind
  `behavioral_signature_service.py` and moved the aggregate body into
  `behavioral_signature_aggregate.py`. `jarvis_tools.py` now retains only the
  compatibility alias plus parked JARVIS executors.
