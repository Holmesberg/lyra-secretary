...
**P0 onboarding ritual shipped — then paused same-day (2026-04-21).** Morning: Alembic migration 025 adds `user.onboarding_completed_at`. Backend `task_manager.create_task` and `create_retroactive_task` stamp the field atomically with the first task insert. `OnboardingFlow` frontend surface replaces `AppShell` rendering when the flag is null — full-screen brain-dump surface, category pre-filled to `planning`.

Late afternoon: dogfood feedback — the brain-dump description had nowhere visible to go on `/today`, and the meta-task was scheduled tomorrow while the user was mid-planning. Attempted mid-flight fix (parse bulleted items → N separate tasks) rejected: fabricates scheduling data. Pivoted to a simpler model — remove the forced onboarding surface, seed a single PLANNED starter task directly inside `resolve_user_from_token` so users land on `/today` with one task waiting. `OnboardingFlow` component + layout conditional + skip-onboarding endpoint all commented out (not deleted) for post-Spring-School re-enable. Import integrations (ICS / Google Calendar / Notion) identified as the real Path B leverage — see `docs/import_integrations_capability_map.md` for the capability map.

Kill criterion unchanged (2026-05-21) — the stamp fires on both the starter-seed path and the regular-create path, so the query doesn't need to distinguish them.

**LYR-100 delete-account hotfix (2026-04-21).** Raw `DELETE FROM user` SQL in the delete endpoint fails on Postgres (reserved keyword). Surfaced when first non-operator user attempted self-delete. Fixed by quoting `"user"`. No data corruption — transaction rolled back atomically.

**Google Calendar read-only integration shipped (2026-04-21 afternoon).** Operator entered Spring School; pulled GCal forward out of the post-Spring-School import roadmap. Scope: read-only sync, events rendered as grey background blocks on `/calendar`, never persisted as `task` rows (research-integrity: imported events excluded from H1 test set by natural design — they don't live in the `task` table). Alembic 026 adds `user.google_refresh_token` (plaintext v1, Fernet deferred). NextAuth scope expanded to `calendar.readonly` with `access_type=offline` + `prompt=consent`. Server-side `/api/calendar/setup` route reads the NextAuth JWT and forwards `refresh_token` to Lyra backend (client JS never touches the token). `services/calendar_sync.py` handles access-token refresh + Redis 60s event cache + graceful 401-to-clear-stored-token. `/calendar` Schedule-X merges Lyra tasks with GCal events via a new `google_external` calendar entry (muted grey + disableDND + disableResize). First Lyra feature to require user re-consent with Google. See `docs/strategic_decisions_april_21.md` §6 for full rationale + pre-registered kill criteria (IMP-1 scope inflation on imported data, IMP-3 retention lift at n≥20 connected users, trust kill at >30% revoke within 7d). V2 deferred items: conflict-detector extension for GCal overlap, `/today` schedule section, Fernet encryption.

**LYR-101 categories fix (2026-04-21, bundled with GCal push).** Custom categories created via "+ Create a new category…" were persisted on the task but invisible in subsequent dropdowns AND rendered with no color badge (CATEGORY_COLORS lookup returned undefined). Fix: `getCategoryColor(cat)` helper hashes custom names into a 10-entry palette, shared `CategorySelect` component fetches `/v1/users/me/categories` which returns built-in + user's distinct non-voided custom categories. See §7.

**LYR-102 colon-in-event-id crash (2026-04-21).** Once the operator enabled Calendar API in Cloud Console and GCal events actually returned, Schedule-X rejected `gcal:<id>` event ids because `:` is a CSS pseudo-class delimiter — `document.querySelector` parsing throws → React crashes the page. Fix: swap separator `:` → `-`. Lesson logged for future integrations: any id passed to Schedule-X (or anything using document.querySelector) must match `[a-zA-Z_][a-zA-Z0-9_-]*`.

**/today + attendance self-report shipped (2026-04-22).** GCal events now inline into /today's feed alongside Lyra tasks — futures in the top bucket with PLANNED (asc by start), past events in the bottom bucket with EXECUTED/SKIPPED (desc by end). Past events carry a "Did you attend?" yes/no control that writes to a new `external_event_outcome` table (migration 027) — not `task`, preserving the H1 research-integrity separation. VT-23 pre-registered for external-source attendance self-report; kill criterion at n≥20 connected users if <15% of past events get marked within 7 days.

**Modal three-bug pass (2026-04-22).** Dogfood surfaced: (1) AM/PM suggestion fires on 0-duration ranges because `negDiffMin > 0` guard fails closed at zero — fix: strict `>= 0` + same-day shifted check; (2) no recovery path when start drifts into the past — new inline "push to HH:MM?" hint below the start field that offers the next 5-min mark; (3) calibration nudge re-suggests after user accepts because the useEffect re-runs with the new duration — fix: `nudgeDecisionMade` flag suppresses future fetches once user has acted (accept OR dismiss), resets on modal re-open.

**Identity-Authorization split — Integrations panel shipped (2026-04-22).** NextAuth sign-in scope stripped from `openid email profile + calendar.readonly` back to `openid email profile` — identity-only, non-sensitive, no OAuth verification required. Every third-party scope is now acquired via incremental OAuth from a new Settings → Integrations panel, one integration per card. Google Calendar is the first live integration; Notion and ICS ship as "Coming soon" preview tiles to communicate the roadmap. New routes: `/api/integrations/google-calendar/connect` (signed state, 10-min TTL, purpose=`gcal_connect`) and `/callback` (state verify + code exchange + id_token email-match guard + backend store). New backend endpoint `GET /v1/integrations` returns per-user status per registered provider; forward-compatible shape for future generic `integration_connection` table. Legacy `/api/calendar/setup` route deleted. Backward compat preserved — existing connected users (operator) keep their refresh_token without re-doing OAuth. Zero DB migration. Triggered by 2026-04-22 retention audit showing 0/5 external users had connected calendar (scope was pure drag), and 2/5 bounced before onboarding with calendar consent as a plausible contributor. See `docs/integrations_architecture.md` for the canonical pattern and `docs/strategic_decisions_april_22.md` for rationale + OAuth-verification deferral. Publishing deferred until VT-23 clears threshold OR ≥10 returning users, whichever first.

**Starter-task scheduler fix (2026-04-22).** `_seed_starter_task` now lands "Plan your week — brain dump and triage" on today, not tomorrow. Three-tier policy: pre-9am signups → today 9am (morning anchor); 9am–23:29 signups → next :30 or :00 mark from now (always strictly future); 23:29+ signups → tomorrow 9am (less than 30 min remaining). End clamps to 23:59 so the window never crosses midnight (would route the task onto tomorrow's /today feed and defeat the fix). Planned_duration follows the clamped window (23:00 signup yields a 29-min slot, not 30). Operator dogfood triggered this: before the fix, every new user saw an empty /today on signup day because the starter was scheduled for the next day — plausible contributor to the 29% signup-to-no-onboarding bounce (u3, u7). Affected existing users: u2 and u7 have `onboarding_completed_at IS NULL` AND 0 tasks, so they'll re-seed with the new logic on next sign-in. u3 same shape but also re-seeds on next sign-in. Research-integrity note: the fix changes when users receive their first planning prompt, which may shift the distribution of `first_task_executed_at - signup_at` that feeds the 2026-05-21 kill-criterion query. The prompt itself (title, category, duration-suggestion) is unchanged. New test file `backend/tests/test_starter_task_schedule.py` (15 cases, covers morning anchor + mid-day + late-night cutover + strict-future invariant + onboarding stamp).

**VT-17d + retroactive pause chip + one-tap quick pause (2026-04-22 evening).** Operator observed two VT-17 predictions correctly anticipating food breaks on 2026-04-21 and 2026-04-22 (clock_anchor mechanism, confidence 0.65 + 0.69, sample sizes 7 + 9). Both logged as `no_response` because operator took breaks outside the app and never clicked Pause — the frozen VT-17 acceptance formula requires a `pause_event` row in the window. This is an in-app-capture gap, not a prediction failure. Ship: (a) retroactive confirmation chip on /today that appears only when `user_response='no_response'` AND no `pause_event` within ±10 min of predicted_at, with Yes/No/dismiss one-tap actions; (b) one-tap quick-pause primary on the in-the-moment `PausePredictionBanner` that defaults pause_reason=intentional_break and skips the reason picker (secondary "Pause + pick reason" still opens the picker); (c) alembic 030 adds `pause_event.self_reported_retroactively` bool; (d) predictor training (`clock_anchor` + `work_rhythm` mechanisms) + VT-17 primary reconciliation both exclude retroactive-flagged rows to prevent self-reinforcement; (e) MANIFESTO v1.9 pre-registers VT-17d permissive acceptance-rate formula with its own window start, frozen at launch. Backend: `POST /v1/pause_predictions/{id}/confirm`, `GET /v1/pause_predictions/pending-confirmation`. Frontend: `PauseConfirmChip` component rendered inline next to the matching task row or as standalone banner if no task_id / task not in current feed. 24h chip-freshness window. Closes feedback-loops-closure-plan Loop 9 (one of the two blocked loops — OpenClaw gating resolved via in-app retroactive path instead of Telegram).

**Archetype clustering Wave 2 shipped (2026-04-22 late-evening, immediately after Wave 1).** Survey UI + Settings profile section land. Same day as Wave 1 because operator dogfood signal ("doesn't REALLY get me") made cold-start personalization time-critical for the next external signup (Omar expected tonight). Scope: (a) `POST /v1/users/me/archetype/survey` scores the 29-item battery (MEQ-5 + BFI-10 C + BSCS-Brief + GP-Short) using server-side reverse-keying, assigns archetype, writes `ArchetypeAssignment` with `completed=True` + `raw_responses` preserved for Gate-3/4 re-scoring; (b) `POST /archetype/skip` defaults to `diffuse_average` with `completed=False` + `skipped_at` stamped; (c) alembic 032 adds graveyard `user.archetype_retrofit_dismissed_at` column + endpoint (unused by UI after same-day refactor); (d) `GET /v1/users/me` adds 5 archetype fields driving the frontend gates. Frontend: `frontend/lib/archetype.ts` carries the 29-item bank with approximated instrument wording (operator to verify verbatim licensing before public beta per strategic_decisions §5.6); `ArchetypeSurvey` 4-screen carousel component with skip-visible-on-every-screen, 2-click skip confirmation, radio-style answer buttons (not sliders — reduces mid-range clustering), reverse-keying invisible to user; `(app)/layout.tsx` gate between ConsentModal and TutorialOverlay fires the survey for post-launch users; `ArchetypeProfileSection` on Settings handles 4 states (no_assignment / skipped / recent_completion / aged_completion) with primary-vs-secondary button prominence, implementing the operator's "allow button in settings to try later" + "allow retry after 3 months" requirements as visual emphasis rather than separate surfaces. Re-takes write new ArchetypeAssignment rows; history preserved for Gate-5 stability analysis. 6 new backend integration tests (303 total passing). No feature flag — shipped directly; rollback via git revert. Omar + future external signups: next new user after this push triggers the survey at onboarding.

**Archetype-prior shrinkage Wave 1 shipped (2026-04-22 late-evening).** Triggered by external trusted-user feedback: *"You know how ChatGPT just gets you? This app understands some of my gaps but doesn't REALLY get me."* Operator pulled the clustering survey + shrinkage blend forward from Phase 5 (post-May 1) into trusted-user week 2. Wave 1 ships the **silent-shrinkage half** of the plan — the canonical bias_factor computation (MANIFESTO v1.10 Rule 13) is now the Rule-13-frozen blend of an archetype-scaled research prior with personal-cascade data, weighted by `min(1.0, n_sessions_in_cell / 30)`. Wave 2 (questionnaire UI + Settings retrofit banner) ships in a following commit chain. Backend: alembic 031 adds `completed` + `skipped_at` + `raw_responses` to `archetype_assignment`; new `archetype_service.py` implements MEQ-5 / BFI-10 C / BSCS-Brief / GP-Short scoring + discipline_z composite + archetype assignment with specific-first pattern matching (45 unit tests covering boundary values, reverse-keying, all 5 archetype cells); `bias_factor_service.py` extracted from `analytics.py` as refactor (zero behavior change at extraction) then extended with the canonical `blend()` function (11 fixture tests: n=0, n=15, n=30 acceptance criteria + archetype fallback + composite scaling math). `GET /v1/analytics/bias_factor/lookup` now returns 8 Rule-13 fields alongside the cascade metadata. Frontend: new-task-modal calibration-nudge reads `bias_factor_final` when present, keeps `cell.bias_factor` as diagnostic. Live verification on operator data (u1 development/morning, n=4): raw personal bias_factor 2.259 (noisy-high on small sample) → blended 1.601 at `personal_weight=0.133`, shrinkage correctly pulling the small-sample value 73% toward the research prior. Every user (new and existing) now benefits from the Diffuse Average default (1.30 × category research prior) instead of flat-1.0 fallback — no survey-take required. Research-integrity: Rule 13 pre-registered in a commit BEFORE any shrinkage-blended value reached a user-facing surface. VT-25 (archetype-reveal narrative internalization) drafted inactive for v1.1. 297 backend tests passing. See `docs/strategic_decisions_april_22.md §5` for acceleration rationale + Gate-1-via-lit-proxy + kill criterion (survey retention-cost floor at 55% signup-to-onboarding).

**LYR-103 delete-account FK hotfix (2026-04-22).** Migration 027 (external_event_outcome, shipped yesterday with GCal /today integration) used a bare `ForeignKey("user.user_id")` — default ON DELETE NO ACTION, so any marked attendance outcome row blocked user deletion. Surfaced when operator tried to delete alt account u2 (asabryhafez) which had one outcome row left over from GCal dogfood. Backend raised `ForeignKeyViolation`, client saw "fails to fetch" because the 500 left the connection mid-response. Fix: alembic 028 adds `ON DELETE CASCADE` to the FK + `delete_my_account` now explicitly `DELETE FROM external_event_outcome WHERE user_id = :u` in both retention and hard-delete branches, matching the pattern used for every other user-scoped table. Belt-and-suspenders: CASCADE is the DB-level backstop for any future code path that forgets the explicit delete; explicit delete works on SQLite test DB (FKs not enforced by default). Regression test `test_delete_account_with_external_outcomes.py` pins both branches. Follow-up tracked in LYR-103: at n<20 outcome rows total in prod the VT-23 aggregate signal loss from purging on retention delete is negligible; when n grows, revisit by adding `post_deletion_retained_at` + `original_user_id_hash` columns to this table (mirroring alembic 019's pattern for task + stopwatch_session) and switching the retention branch to anonymize instead of purge.
