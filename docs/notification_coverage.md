...
| Pause prediction | yes | operator-owned full text; non-operator redacted metadata only | one per canonical open session; 10m fallback cooldown; 30m across prediction families; none 22:00-07:59 user-local | Candidate scan requires an open, non-auto-closed, non-DQ session. Shared spacing serializes on the existing user row and treats either durable firing family as the conservative reservation boundary. Quiet hours use the persisted user timezone and invalid timezone data fails closed; raw task ids/titles stay out of the shared bot. |
| Resume prediction | yes | operator-owned full text; non-operator redacted metadata only | 60m; maximum two per paused session; 30m across prediction families; none 22:00-07:59 user-local | Shared spacing serializes on the existing user row and treats either durable firing family as the conservative reservation boundary. Quiet hours use the persisted user timezone and invalid timezone data fails closed. Raw task title and ids stay in the per-user queue; the operator mirror sees redacted metadata only. |
