...
**P0 onboarding ritual shipped (2026-04-21).** Alembic migration 025 adds `user.onboarding_completed_at`. Backend `task_manager.create_task` and `create_retroactive_task` stamp the field atomically with the first task insert. New `POST /users/me/skip-onboarding` endpoint for the explicit opt-out path. Frontend: `OnboardingFlow` component replaces `AppShell` rendering when the flag is null — full-screen brain-dump surface, category pre-filled to `planning`, tomorrow 9am / 30 min defaults. First Path B infrastructure on the wire. Existing 5 users with historical tasks backfilled; 2 users with zero tasks will hit the surface on next sign-in. Kill criterion reads 2026-05-21.
