...
- **Conflict detection with forced override affordance** — Finding 1 implementation. Gate B applies hard block on active overlap, hard block on non-voided overlap, and soft warning on duplicate title. 409 responses include an `override_token` with 60-second validity. Frontend surfaces `[Cancel]` / `[Override]` affordance; override actions log the override reason for analytics (override rate per gate is tracked — see dogfood §Conflict detection override rate monitoring). Preserves research integrity while respecting user authority.
