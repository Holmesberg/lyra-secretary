...
- **Conflict detection with forced override affordance (April 16 relaxation — Path A).** Finding 1 implementation, walked back from yesterday's "hard block on any non-voided overlap" per Apr 15 dogfood finding. Severity model: **HARD** = overlap with state=EXECUTING (single-mutation-authority, force=true cannot override); **SOFT** = overlap with PLANNED or PAUSED (force-overridable; PAUSED still routes through the existing interruption-flow first); **SOFT** = same title (case-insensitive) on the same UTC date (force-overridable). Each conflict carries a `gate_id` (`active_overlap` | `planned_overlap` | `duplicate_title`) so the dogfood Day-10 override-rate analytics can attribute overrides per gate. Frontend surfaces `[Cancel]` for HARD (no override possible — must stop the active timer first) and `[Cancel] / [Create anyway]` for SOFT. The `override_token` 60-second one-shot from the original spec is **deferred** — current implementation uses the simpler `force=true` boolean already in the wire contract; replay-protection token is its own design pass. Allowing planned-task overlap is a precondition for the multi-task logging investigation in `parked_ideas.md`. Shipped commit `bb5d5d9`.
