...
12. **Scope inflation mediation test (VT-22 falsification).** *(Added April 17, 2026.)* When `task.description` data is available (≥ 50 tasks with non-empty descriptions per user), compute `scope_density = description_item_count / planned_duration_minutes` for each task. Run a mediation analysis: `pre_task_readiness → scope_density → duration_delta_minutes`. **If scope_density mediates the relationship** (readiness → delta attenuation ≥ 40% when controlling for scope_density, Sobel test p < 0.05): delta is primarily a scope inflation signal, not a time estimation error signal. Calibration nudge architecture pivots from time-adjustment to scope-adjustment in Phase 6+. **If scope_density does NOT mediate** (attenuation < 20%): the standard time-estimation frame stands, VT-22 is falsified, and the description field remains a Phase 5 corpus accumulator without elevated measurement status. **Interim analysis (before brain dump data):** use `pause_count` and within-session duration variance as proxy scope-complexity indicators. Report the readiness → proxy_complexity → delta mediation path as an exploratory supplement. This interim analysis is NOT a basis for the pivot decision — only the brain-dump-based mediation is authoritative.
    ...
    **Amendment (2026-04-26 — Loop 11 schema lands).** With Alembic 033, `description_item_count` is now operationally defined as the column `task.scope_bullet_count_at_plan` (auto-counted bullet markers `^\s*[-*•·]` in `task.description` at PLANNED-state creation, captured by `extract_scope_bullets()` in `backend/app/services/parser.py`). Replace `description_item_count` with `scope_bullet_count_at_plan` in the ratio: `scope_density = scope_bullet_count_at_plan / planned_duration_minutes`. The execute-time re-sample column `scope_bullet_count_at_execute` enables a paired secondary analysis (within-task scope drift = `at_execute − at_plan`) that is exploratory, NOT part of the kill criterion. **Stratification addendum:** when ≥ 50 deadline-bound tasks exist per user (`task.deadline_id IS NOT NULL`, `voided_at IS NULL`), rerun the mediation stratified by deadline binding (`deadline_id IS NULL` vs `IS NOT NULL`). If scope inflation is deadline-driven, the IS NOT NULL arm should show stronger mediation (larger attenuation). This stratified result is reported alongside the unstratified one but does NOT supersede the primary kill criterion.
...
17. **VT-25 archetype-reveal measurement (no-RCT design).** *(Added April 27, 2026 — pre-registered alongside the dynamic-reveal ship. Operator decision: ship dynamic reveal to all users without a control arm; accepts that VT-25 §25b becomes untestable in this design. See AskUserQuestion 2026-04-27.)*
    ...
    **Activation status (RETROACTIVE).** VT-25 was activated 2026-04-26 by the LYR-112 reveal commit (`3c459b0`) — that commit shipped the static "Profile: Procrastinator" label at session 5+ to two users (operator + one survey completer), but no kill criteria were frozen at that moment. Rule 17 closes the gap. The static reveal is now superseded by the dynamic posterior reveal (this commit's `archetype_proximity_service` + endpoint + frontend). Both pre-existing static-exposed users will see the dynamic reveal at next page load.

    **§25a kill criterion (FROZEN, ACTIVE).** For users whose **first reveal exposure** is the dynamic posterior reveal (i.e., NEW users joining post Rule-17 ship), compute the within-user shift in mean `planned_duration_minutes` over the 10 EXECUTED, non-voided sessions preceding vs the 10 sessions following the user's first dynamic-reveal impression, stratified by the archetype carrying the highest posterior score at first reveal (= the archetype the user "looks most like" initially), controlling for category and time_of_day.

    **Detection:** users for whom the highest-scoring archetype is high-prior (`procrastinator` 1.80, `lark_low_discipline` 1.50, `diffuse_average` 1.30) AND who inflate post-reveal `planned_duration_minutes` by ≥ 15% relative to pre-reveal in those archetypes' canonical (category × tod) cells → **label reinforcement confirmed**. Mitigation: demote the dynamic-reveal copy further (drop archetype names entirely; show only abstract pattern shapes — "your tendencies are stabilizing toward a low-discipline morning rhythm" rather than naming Procrastinator).

    **Naming note — "reinforcement" not "internalization" (refined 2026-04-27 per operator).** The original VT-25 framing called this same signal "label-internalization." That phrase presumes the label was NEW to the user when Lyra surfaced it — i.e., that Lyra TAUGHT the user they're X. But users classified as a high-prior archetype (e.g., Procrastinator) often already self-identify as that archetype before any Lyra reveal. In that case the 15% post-reveal inflation is **reinforcement of an existing self-image** (or release-from-restraint — "now I have permission to plan loosely") rather than installation of a new label. §25a's metric (15% inflation in high-prior assignments, n ≥ 5 fresh-exposure users floor) does not change; only the causal-story label widens. The mitigation (drop archetype names from copy) applies identically to all three plausible causal stories: genuine internalization, reinforcement of pre-existing self-image, or permission-granting. The §25a writeup must acknowledge this triple-confound when reporting results — we cannot claim "Lyra caused this" without an additional pre-reveal self-classification instrument the cohort hasn't filled out.

    **§25b kill criterion (DROPPED — untestable in this design).** §25b requires a comparison between users who saw the reveal and users who did not. The no-RCT operator decision means every user receives the dynamic reveal; there is no suppressed arm. §25b is therefore untestable until a future split-cohort revisit. Documented here so the gap is explicit and cannot be silently re-opened later: any future "did the dynamic reveal bend behavior?" claim REQUIRES a fresh pre-registered RCT with a no-reveal arm.

    **Cohort caveat (the 2 pre-existing static-exposed users).** Operator (`user_id=1`) and the second post-Apr-26 archetype-survey completer had their first reveal exposure be the LYR-112 static "Profile: Procrastinator" copy, not the dynamic posterior. Their data is **excluded from §25a's pre-vs-post comparison** because they were contaminated by static-reveal exposure first. They still receive the dynamic reveal at Rule-17 ship — the exclusion is analysis-side only, not a UX gate. Operationally: §25a queries filter `WHERE user.created_at > '2026-04-27 [Rule 17 commit timestamp]'` to exclude both pre-existing exposed users.

    **Floor on activation.** §25a does not run until ≥ 5 users qualify (first exposure was dynamic AND ≥ 10 EXECUTED sessions before the first reveal AND ≥ 10 EXECUTED sessions after). This is a hard data-floor, not a time-floor. Below the floor, §25a returns "insufficient power" and any narrative claim about VT-25 must explicitly say so.

    **No post-hoc threshold tuning permitted.** Once §25a fires (n ≥ 5 qualifying users), the 15% threshold is frozen.

    **Implementation anchor.** The dynamic-reveal endpoint is `GET /v1/analytics/archetype/proximity` (and `/v1/analytics/archetype/proximity/trend`) in `backend/app/api/v1/endpoints/analytics.py`, backed by `backend/app/services/archetype_proximity_service.py`. The first-reveal-impression timestamp is captured in `reflection_view_log` with `reflection_type='archetype_proximity'`. §25a aggregation lives in the operator notebook, not in the application code — keeping the analysis layer separable from the surface layer per the existing VT-17/VT-21 pattern.

    **Math amendment v1.15 (April 27, 2026) — effective-sample-size correction + bias-factor winsorization.** Two changes to `_compute_log_posteriors`. (a) Each archetype's `Σ log_likelihood` is divided by `sqrt(N_used)` before the log-prior is added — Kish-style effective-sample-size treatment, treating the cluster of within-user-and-window tasks as ~sqrt(N) effective independent observations rather than N (the iid assumption is structurally false: same user × same week × same project × same measurement protocol → strong within-cluster correlation). (b) Each task's `observed_bf = executed/planned` is winsorized to `[0.30, 3.0]` before the Normal-pdf evaluation. Operator dogfood revealed extreme outliers (bf 9.2, 3.7, 3.5) that drove tight-σ archetypes to log-likelihood -∞; the only archetype wide enough to absorb a 9× overrun (procrastinator σ=0.40) was winning by a margin no amount of ESS damping could close. Bf > 3.0 is no longer informative about archetype identity — it falls under VT-22 (scope inflation), which has its own pre-registered mediation test (analysis rule 12). The §25a metric, kill threshold (15% post-reveal planning inflation), floor (n ≥ 5 fresh-exposure users), and cohort caveat (2 pre-existing static-exposed users) are all unchanged. Analyses produced under v1.13/v1.14 (the un-damped, un-winsorized formula) remain on record and are tagged with their math version. UI display cap (99% maximum) and floor (2% minimum bar width) added in `archetype-proximity-display.tsx` as belt-and-suspenders against future saturation; rendering layer only, no math impact.
