...
silently complete LyraOS deadlines; the user remains author of canonical
...
ARCHITECTURE — course-code matching (operator decision 2026-05-01):
  LyraOS deadline matches a Moodle assignment ONLY when they share the
...
MATCHING:
  LyraOS deadline title + due_at_utc → best Moodle assignment via
  (title_similarity + due_proximity) score. Each Moodle assignment
  binds to AT MOST ONE LyraOS deadline (greedy first-served by score).
...
# LyraOS's deadline table (operator request 2026-05-01 — "submitted
# tasks should pop up", "could LyraOS pick up unsubmitted as overdue").
...
def submission_completed_at(status_resp: dict) -> Optional[datetime]:
    ...
    This is a Moodle submission trace, not evidence of LyraOS task execution.
...
def sync_user(user: User, base_url: str, db: Session) -> SubmissionSyncResult:
    ...
    with at least one BOUND (non-voided) LyraOS task are synced. Unbound
    ...
    # existing LyraOS deadline, create one. Operator request 2026-05-01:
    # "submitted tasks should pop up in deadlines" + "could LyraOS pick
    ...
    # but only 4 deadlines in LyraOS. This backfills from WS so the
