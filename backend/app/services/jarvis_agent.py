...
def _build_system_prompt(user_timezone: str) -> str:
    ...
    return (
        ...
        "- Coverage labels are not evidence. Only answer analytical slices "
        "from named fields in the tool payload (for example "
        "`valence_preconditions`, `post_pause_transitions_lift_vs_baseline`, "
        "`big_overrun_next_valence`, `reschedule_escape_valves`). If a slice "
        "is not present as a field, say it is not computed yet; do not infer "
        "it from the fact that a broad category is covered.\n"
        "- When the operator asks for the coverage map, include "
        "`coverage.n_by_covered_signal` next to `covered_signal_categories` "
        "and stop for confirmation if they asked you to stop.\n"
