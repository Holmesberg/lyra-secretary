 ...
 *   3. Save the downloaded JSON or copy __LYRA_SHADOW_EXPORT_JSON__.
 *
...
(async () => {
  ...
  const EXPORT_VERSION = "lyra-client-shadow-readonly-v1";
  ...
  function saveExport(payload) {
    ...
    const fileName = `lyra-client-shadow-export-${startedAt.replace(/[:.]/g, "-")}.json`;

    globalThis.__LYRA_SHADOW_EXPORT__ = payload;
    globalThis.__LYRA_SHADOW_EXPORT_JSON__ = json;

    if (typeof globalThis.copy === "function") {
      try {
        globalThis.copy(json);
        console.info("Lyra shadow export JSON copied to clipboard by DevTools copy().");
      } catch (error) {
        console.warn("DevTools copy() failed; use __LYRA_SHADOW_EXPORT_JSON__.", error);
      }
    }
    ...
    if (globalThis.document && typeof Blob !== "undefined" && globalThis.URL) {
      ...
      console.info(`Lyra shadow export download requested: ${fileName}`);
      return;
    }

    console.info("No document download context. Read __LYRA_SHADOW_EXPORT_JSON__.");
  }
  ...
  const payload = {
    ...
    read_only: true,
    warning:
      "This file was generated by the canonical Lyra read-only client shadow exporter.",
