/*
 * Barzakh client shadow export, read-only.
 *
 * Usage:
 *   1. Open DevTools in the page or extension context that may contain data.
 *   2. Paste this exact file contents.
 *   3. Save the downloaded JSON or copy __BARZAKH_SHADOW_EXPORT_JSON__.
 *
...
(async () => {
  ...
  const EXPORT_VERSION = "barzakh-client-shadow-readonly-v1";
  ...
  function saveExport(payload) {
    ...
    const fileName = `barzakh-client-shadow-export-${startedAt.replace(/[:.]/g, "-")}.json`;

    globalThis.__BARZAKH_SHADOW_EXPORT__ = payload;
    globalThis.__BARZAKH_SHADOW_EXPORT_JSON__ = json;

    if (typeof globalThis.copy === "function") {
      try {
        globalThis.copy(json);
        console.info("Barzakh shadow export JSON copied to clipboard by DevTools copy().");
      } catch (error) {
        console.warn("DevTools copy() failed; use __BARZAKH_SHADOW_EXPORT_JSON__.", error);
      }
    }
    ...
    if (globalThis.document && typeof Blob !== "undefined" && globalThis.URL) {
      ...
      console.info(`Barzakh shadow export download requested: ${fileName}`);
      return;
    }

    console.info("No document download context. Read __BARZAKH_SHADOW_EXPORT_JSON__.");
  }
  ...
  const payload = {
    ...
    read_only: true,
    warning:
      "This file was generated by the canonical Barzakh read-only client shadow exporter.",
